...
class IMDBDataset(Dataset):
    def __init__(self, texts, labels, tokenizer, max_length=492):  # 512 - prefix_size
...
class PrefixEncoder(nn.Module):
    def __init__(self, prefix_size, hidden_size, prefix_projection=False):
        super().__init__()
        ...
        if prefix_projection:
            ...
            self.embedding = nn.Embedding(prefix_size, hidden_size)
    
    def forward(self, prefix_ids=None):
        if self.prefix_projection:
            # Generate random embeddings and project them
            prefix_embeddings = torch.randn(
                self.prefix_size, 
                self.hidden_size, 
                device=self.projection[0].weight.device
            )
            projected = self.projection(prefix_embeddings)
        else:
            if prefix_ids is None:
                prefix_ids = torch.arange(self.prefix_size, device=self.embedding.weight.device)
            projected = self.embedding(prefix_ids)
        
        return projected


class PrefixTuningModel(nn.Module):
    def __init__(self, base_model, prefix_size=20, prefix_projection=False, max_length=512):
        super().__init__()
    ...
    def get_prompt_params(self):
        return {
            ...
            if 'prefix_encoder' in k or 'classifier' in k
        }
    ...
    def forward(self, input_ids, attention_mask):
        ...
        prefix_embeddings = self.prefix_encoder()  # (prefix_size, hidden_size)
        
        # Get input embeddings from the model
        input_embeds = self.base_model.get_input_embeddings()(input_ids)
        
        # Calculate available sequence length for inputs
        available_length = self.max_length - self.prefix_size
        input_embeds = input_embeds[:, :available_length, :]
        attention_mask = attention_mask[:, :available_length]
        
        # Expand prefix embeddings for batch size
        ...
        prefix_attention_mask = torch.ones(batch_size, self.prefix_size, device=device)
        ...
        pooled_output = sequence_output[:, self.prefix_size]  # Use first token after prefixes
...
class FedAvgServer(BaseServer):
    ...
    def aggregate_models(self, client_models, aggregation_type="fedavg"):
        ...
        for key in aggregated_dict.keys():
            if 'prefix_encoder' in key or 'classifier' in key:
    ...
    def __init__(self, model, train_dataset, val_dataset, device, client_id, **kwargs):
        ...
        self.optimizer = torch.optim.AdamW(
            [p for n, p in self.model.named_parameters() 
             if 'prefix_encoder' in n or 'classifier' in n],
            lr=2e-5
        )
...
class MOONClient(BaseClient):
    def __init__(self, model, train_dataset, val_dataset, device, client_id, temperature=0.5, **kwargs):
        ...
        self.projection = nn.Sequential(
            nn.Linear(model.hidden_size, 256),
            nn.ReLU(),
            nn.Linear(256, 128)
        ).to(device)
        self.optimizer = torch.optim.AdamW(
    ...
    def train(self, global_params=None, aggregation_type="moon", epochs=1, batch_size=8):
        ...
        for epoch in range(epochs):
            for batch in train_loader:
                ...
                outputs_current = self.model.base_model(input_ids, attention_mask)
                outputs_prev = self.previous_model.base_model(input_ids, attention_mask)
                outputs_global = global_model.base_model(input_ids, attention_mask)
                
                current_features = outputs_current.last_hidden_state[:, 0]
                prev_features = outputs_prev.last_hidden_state[:, 0].detach()
                global_features = outputs_global.last_hidden_state[:, 0].detach()
...
class FedDynClient(BaseClient):
    ...
    def train(self, global_params=None, aggregation_type="feddyn", epochs=1, batch_size=8):
        ...
        initial_params = {
            ...
            if 'prefix_encoder' in name or 'classifier' in name
        }
        ...
        for epoch in range(epochs):
            for batch in train_loader:
                ...
                for name, param in self.model.named_parameters():
                    if 'prefix_encoder' in name or 'classifier' in name and param.grad is not None:
        ...
        for name in param_gradients:
            param_gradients[name] = param_gradients[name] / num_steps
...
def main():
    ...
    for experiment_num in range(experiment_start, experiment_end + 1):
        ...
        peft_method = f"prefix{experiment_num}"
            
        # Initialize logging and checkpointing
        logger = FederatedLogger(base_dir=os.path.join("federated_logs", peft_method))
        ...
        dataset = load_dataset("imdb", cache_dir="/home/FYP/ytan232/.cache/huggingface/datasets")
        ...
        # Initialize base model and prefix tuning model
